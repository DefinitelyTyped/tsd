...
export class LineParserCore {
	...
	getInfo() {
		var ret: any = {};
		ret.parsers = collection.keysOf(this.parsers).sort();
		return ret;
	}
	...
	private link() {
		this.parsers.forEach((parser: LineParser) => {
			Lazy(parser.nextIds).each((id: string) => {
	...
	get(ids: string[]): LineParser[] {
		return Lazy(ids).reduce((memo: LineParser[], id: string) => {
	...
	all(): LineParser[] {
		return collection.valuesOf(this.parsers);
	}

	listIds(parsers: LineParser[]): string[] {
		return Lazy(parsers).reduce((memo: string[], parser: LineParser) => {
			memo.push(parser.id);
			return memo;
		}, []);
	}
