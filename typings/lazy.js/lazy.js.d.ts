...
declare module LazyJS {
	...
	class Sequence<T> extends SequenceBase<T> {
		each(eachFn: ValueCallback<T>):Sequence<T>;
	}

	class SequenceBase<T> extends SequenceBaser<T> {
		first():any;
	...
	class ArrayLikeSequence<T> extends Sequence<T> {
		// define()X;
	...
	export class ObjectLikeSequence<T> extends Sequence<T> {
	...
	export class StringLikeSequence extends SequenceBaser<string> {
